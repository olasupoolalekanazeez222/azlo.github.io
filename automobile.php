...
if(isset($_POST['ask'])){
	...
	$imagename = $_POST['image'];
		  //echo $user.$email ;
	$f =  $connn->query("INSERT  into askauto  values('$user','$email','','$comment','$time')");
	if($f){
...
if(isset($_POST['sell'])){
	...
	// putting a file into a folder 
	 if(isset( $_FILES['filesell']['name'])){
		  ...
		  //echo $user.$email ;
		$there =   $connn->query("select * from automobile where person='$user'");
		  if($there->num_rows){
			  
			  echo '<script>alert("use some other username")</script>' ;
		  }
		  else{
	$f =  $connn->query("INSERT  into automobile  values('$user','$email','$amount','$comment','$product','$phone')");
	if($f){
...
$res = $conn->query("select * from automobile");
if($res->num_rows){
