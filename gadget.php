...
if(isset($_POST['ask'])){
	...
	$imagename = $_POST['image'];
		  //echo $user.$email ;
	$f =  $connn->query("INSERT  into askgadget  values('$user','$email','','$comment','$time')");
	if($f){
...
if(isset($_POST['sell'])){
	...
	// putting a file into a folder 
	 if(isset( $_FILES['filesell']['name'])){
		  ...
		  //echo $user.$email ;
		$there =   $connn->query("select * from gadget where person='$user'");
		  if($there->num_rows){
			  
			  echo '<script>alert("use some other username")</script>' ;
		  }
		  else{
	$f =  $connn->query("INSERT  into gadget  values('$user','$email','$amount','$comment','$product','$phone')");
	if($f){
...
<br/>
 ...
 about gadget anywhere in nigeria and get it deliver to you in few minutes
    ...
    <form   method="post" action="gadget.php" id="cask"   style="visibility:visible;">
...
$res = $conn->query("select * from gadget");
if($res->num_rows){
for($i=0; $i<$res->num_rows; $i++ ){
$row = $res->fetch_array(MYSQLI_ASSOC);
echo "<center><div id='sule' style='background-color:#f5f5f5; width:60%; margin-left:30%; margin-top:20px; height:600px;'><a href='estate.php?ime=".$row['product']."'><img src='input/".$row['person']."es.png' style='height:400px; margin-top:20px;width:80%;'></a><br/><br/>".$row['product'].'<br/>phone number:&nbsp;'.$row['phone'].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;amount:&nbsp;'.$row['amount'].'<br/>description:&nbsp;'.$row['description']."<br/><button style='background-color:purple; margin-top:10px; border-color:purple;'>buy now</button><br/></div></center>";
echo <<<_END
<script>
...
?>
  ...
  <!-- website review will be shown here-->
	<div id="use" style="visibility:hidden;" >hide</div>
	<div id="ause" style="visibility:hidden;">hide</div>
  <footer>
  <center>
  ...
  function sell(){
	  //let sell = document.getElementById("sell");
	   let csell = document.getElementById("csell");
	  if( document.getElementById("use").textContent =='hide'){
		  document.getElementById("use").textContent ='unhide';
		  csell.style.visibility="visible";
	  }
	  
	  else if(document.getElementById("use").textContent =='unhide'){
		  
		  document.getElementById("use").textContent ='hide';
		  csell.style.visibility="hidden";
	  }
  }
  ...
  function ask(){
	...
	//let sell = document.getElementById("sell");
	   let csell = document.getElementById("cask");
	  if( document.getElementById("ause").textContent =='hide'){
		  document.getElementById("ause").textContent ='unhide';
		  csell.style.visibility="visible";
	  }
	  
	  else if(document.getElementById("ause").textContent =='unhide'){
		  
		  document.getElementById("ause").textContent ='hide';
		  csell.style.visibility="hidden";
	  }  
  }
